...
export default function ArtKG() {
  ...
  const handleSave = async () => {
    try {
      await fetch('http://localhost:4000/api/KG/Art', {
        method: 'POST',
  ...
  const handleScreenshot = () => {
    html2canvas(document.body).then(canvas => {
      ...
      const fileName = `kg_art_${timestamp}.png`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch('http://localhost:4000/api/KG/Art');
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#fff' }}>
      ...
      <h2 style={{ textAlign: 'center', marginBottom: '1rem', color: '#000' }}>ART Class</h2>
